...
async function loadCall(i){
 ...
 s.promise=(async()=>{
  ...
  const settings=detail('本次请求设置与完整工具定义',Object.fromEntries(Object.entries(req).filter(([k])=>k!=='messages')));settings.classList.add('chat-settings');s.content.append(settings);if(d.native_request_url){const original=detail('实际发送的原生 Responses 请求（完整 JSON）',await get(d.native_request_url));s.content.append(original)}
  ...
  if(d.thinking){bubble.append(el('div','chat-block-label',d.reasoning_visibility==='summary_only'?'服务返回的推理摘要（不是完整内部思考）':'接口返回的思考正文'));const t=code(d.thinking);t.classList.add('chat-reasoning');bubble.append(t)}
