...
try{
 ...
 const choices=idx.cases.flatMap(c=>c.runs.map(r=>({c,r}))).sort((a,b)=>Number(b.c.id.startsWith('active-'))-Number(a.c.id.startsWith('active-')));const chosen=choices.find(x=>x.r.id===params.get('run'))||choices.find(x=>x.r.id==='active-91105-flash')||choices.find(x=>x.c.id.startsWith('active-')&&x.r.model==='flash')||choices[0];
